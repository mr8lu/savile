...
def import_system_cmd(
    name: str = typer.Argument(
        ..., help="Name of the system skill or agent to import (e.g. 'sprint-status')"
    ),
    alias: str = typer.Option(
        None,
        "--alias",
        "-a",
        help="Alias to rename the imported persona/framework file",
    ),
    source_dir: Path = typer.Option(
        None,
        "--dir",
        "-d",
        help="Custom local system directory to look for the skill/agent instead of default ~/.gemini or ~/.agents",
    ),
):
    ...
    try:
        p_file, f_file = registry.import_from_system(
            vault_path, name, alias, source_dir
        )
