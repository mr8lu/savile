    ...
    if not state_file.exists():
        with open(state_file, "w") as f:
            json.dump(
                {
                    "tasks": [],
                    "project_context": "Initialize your project context here.",
                },
                f,
            )
    return state_file
...
async def get_prompt_handler(
    vault_path: Path, name: str, arguments: dict | None
) -> GetPromptResult:
        ...
        Tool(
            ...
            inputSchema={
                ...
                "properties": {
                    "category": {
                        "type": "string",
                        "description": "Either 'personas' or 'frameworks'",
                    },
                    "name": {
                        "type": "string",
                        "description": "The exact filename of the module (e.g., 'realist.md')",
                    },
                },
        ...
        Tool(
            ...
            inputSchema={
                ...
                "properties": {
                    "category": {
                        "type": "string",
                        "description": "Source category: 'personas' or 'frameworks'",
                    },
                    "name": {
                        "type": "string",
                        "description": "The exact filename of the module (e.g., 'pm.md')",
                    },
                    "target_type": {
                        "type": "string",
                        "description": "Target directory type: 'workflow' or 'skill'",
                    },
                    "project_path": {
                        "type": "string",
                        "description": "Optional explicit path to the project directory. Defaults to the CWD of the server if omitted.",
                    },
                },
            ...
            Tool(
                ...
                inputSchema={
                    ...
                    "properties": {
                        ...
                        "description": {
                            "type": "string",
                            "description": "Detailed description, criteria, and context.",
                        },
                        "assignee_persona": {
                            "type": "string",
                            "description": "The persona (e.g. 'dev', 'architect') this should be routed to.",
                        },
                    },
            ...
            Tool(
                ...
                inputSchema={
                    ...
                    "properties": {
                        ...
                        "status": {
                            "type": "string",
                            "description": "Must be one of: 'todo', 'in_progress', 'review', 'done'",
                        },
                        "notes": {
                            "type": "string",
                            "description": "Append any findings, commit hashes, or handoff notes.",
                        },
                    },
...
async def call_tool_handler(
    vault_path: Path, name: str, arguments: dict
) -> list[TextContent]:
    if arguments is None or not isinstance(arguments, dict):
        ...
        if name != "list_logic_modules" or arguments is not None:
            return [
                TextContent(
                    type="text", text="Error: arguments must be a valid dictionary."
                )
            ]
        arguments = {}
    ...
    if name == "list_logic_modules":
        ...
        if category not in ["personas", "frameworks"]:
            return [
                TextContent(
                    type="text",
                    text="Error: Category must be 'personas' or 'frameworks'",
                )
            ]
        ...
        if not target.exists() or not target.is_file():
            return [
                TextContent(
                    type="text", text=f"Error: {module_name} not found in {category}."
                )
            ]
        ...
        if target_type not in ["workflow", "skill"]:
            return [
                TextContent(
                    type="text", text="Error: target_type must be 'workflow' or 'skill'"
                )
            ]
        ...
        if not source_file.exists() or not source_file.is_file():
            return [
                TextContent(type="text", text=f"Error: Source {module_name} not found.")
            ]
        ...
        if target_file.exists():
            return [
                TextContent(
                    type="text",
                    text=f"Error: Target {module_name} already exists. Refusing to overwrite.",
                )
            ]
        ...
        if not task:
            return [
                TextContent(
                    type="text",
                    text="Error: You must provide a 'task' argument for this agent.",
                )
            ]
        ...
        if not target.exists() or not target.is_file():
            return [
                TextContent(
                    type="text",
                    text=f"Error: Agent persona '{persona_name}' not found.",
                )
            ]
    ...
    # --- Symphony Control Plane Handlers ---
        ...
        return [
            TextContent(type="text", text=f"Task added successfully. ID: {task_id}")
        ]
...
async def run_sse_server(vault_path: Path, port: int = 8000):
    ...
    uvicorn_config = uvicorn.Config(
        starlette_app, host="127.0.0.1", port=port, log_level="info"
    )
