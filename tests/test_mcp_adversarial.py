...
async def test_vulnerability_argument_type_validation(mock_vault):
    ...
    try:
        ...
        pytest.fail(
            "Server crashed due to unhandled NoneType argument. Missing type validation."
        )

    try:
        results = await call_tool_handler(
            mock_vault, "read_logic_module", ["invalid", "list"]
        )
        assert len(results) == 1
        assert "Error" in results[0].text
    except AttributeError:
        pytest.fail(
            "Server crashed due to unhandled List argument. Missing type validation."
        )
...
async def test_vulnerability_symlink_path_traversal(mock_vault, tmp_path):
    ...
    results = await call_tool_handler(
        mock_vault,
        "read_logic_module",
        {"category": "personas", "name": "malicious.md"},
    )

    # Enforce that it explicitly blocks symlinks pointing outside the vault
    assert len(results) == 1
    assert "SUPER_SECRET_TOKEN" not in results[0].text, (
        "Path traversal vulnerability: Able to read arbitrary files via symlink"
    )
    assert "Error:" in results[0].text
...
async def test_vulnerability_implicit_cwd_pollution(mock_vault, tmp_path):
    ...
    try:
        ...
        assert target_file.exists(), (
            "Installation target should respect project_path argument"
        )
        assert not (random_dir / ".agent").exists(), (
            "Vulnerability: Tool pollutes CWD despite explicit project_path"
        )
    finally:
...
async def test_vulnerability_silent_overwrite(mock_vault, tmp_path):
    ...
    try:
        ...
        assert existing_file.read_text() == "USER_CUSTOM_WORKFLOW_DO_NOT_OVERWRITE", (
            "Vulnerability: Tool silently clobbers existing user files without warning or force flag."
        )
    finally:
        os.chdir(original_cwd)
...
async def test_vulnerability_bad_frontmatter_parsing(mock_vault, tmp_path):
    ...
    try:
        ...
        results = await call_tool_handler(
            mock_vault,
            "install_logic_module",
            {"category": "personas", "name": "bad.md", "target_type": "workflow"},
        )
        assert len(results) == 1
...
async def test_vulnerability_duplicate_names(mock_vault):
    ...
    assert "Framework Dup" in result.messages[0].content.text, (
        "Vulnerability: Cannot access framework if persona shares the same name (Namespace Shadowing)"
    )
